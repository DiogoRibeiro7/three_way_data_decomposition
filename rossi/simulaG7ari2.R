# simulaG7ari2 function: Simulate ARI results and save to a file
#
# This function simulates ARI (Adjusted Rand Index) results for a specified
# number of scenarios and saves the results to a file named 'G7ari2.RData'.
#
...
#
# Returns:
#   None. The results are saved to 'G7ari2.RData'.

simulaG7ari2 <- function() {
  ns <- 250
  ...
  for (N in c(500, 1000)) {
    for (nrep in c(1, 3)) {
      ...
      for (dgp in 1:4) {
        ...
        # Uncomment the following lines if using Scenario 1
        # ari <- tryCatch(
        #   {
...
determine_r <- function(N, nrep) {
  if (N == 500 && nrep == 1) {
    ...
    return(2)
  } else if (N == 1000 && nrep == 1) {
    return(3)
  } else if (N == 1000 && nrep == 3) {
    return(4)
  } else {
